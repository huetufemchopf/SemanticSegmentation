...
if __name__=='__main__':
    ...
    train_loader = torch.utils.data.DataLoader(data.DataLoaderSegmentation(args, mode='train'),
                                               batch_size=args.train_batch, 
                                               num_workers=args.workers,
                                               shuffle=True)
    val_loader   = torch.utils.data.DataLoader(data.DataLoaderSegmentation(args, mode='val'),
                                               batch_size=args.train_batch, 
                                               num_workers=args.workers,
                                               shuffle=False)
    ''' load model '''
    print('===> prepare model ...')
    model = improved_model.Resnet50_8s_psp(args)

    model.cuda() # load model to gpu
    # model = torch.nn.DataParallel(model,device_ids=list([2,3])).cuda()
    ''' define loss '''
    ...
    for epoch in range(1, args.epoch+1):
        ...
        for idx, (imgs, cls, img_path) in enumerate(train_loader):
