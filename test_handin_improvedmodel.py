...
if __name__ == '__main__':
    ...
    model = improved_model.Resnet50_8s_psp(args).cuda()
#    model = torch.nn.DataParallel(model).cuda()
    model_std = torch.load("./log/model_bestbest.pth.tar", map_location="cuda:0")
    model.load_state_dict(model_std)

    ''' resume save model '''
    checkpoint = torch.load("./log/model_bestbest.pth.tar", map_location="cuda:0")
